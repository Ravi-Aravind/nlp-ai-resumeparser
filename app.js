...
class DocumentParser {
    ...
    extractPersonalInfo(text) {
        ...
        const nameMatches = text.match(/^([A-Z][a-z]+(?:\s+[A-Z][a-z']*){1,3})\s*$/m);
        if (nameMatches) {
            info.name = nameMatches[1].trim();
            info.confidence.name = 90;
        } else {
            // Try alternative name pattern
            const altNameMatch = text.match(/([A-Z][a-z]+\s+[A-Z][a-z]+)/);
            if (altNameMatch) {
                info.name = altNameMatch[1];
                info.confidence.name = 70;
            }
        }
...
function handleFileUpload(files) {
    ...
    updateProcessingStatus(`Processing ${files.length} document(s)...`, 'processing');
...
async function processRealResume(file, container) {
    ...
    try {
        ...
        lucide.createIcons();
        updateProcessingStatus(`Successfully processed ${file.name}`, 'success');
        showToast(`Real data extracted from ${file.name}! 🎉`, 'success');
        ...
        `;
        lucide.createIcons();
        updateProcessingStatus(`Error processing ${file.name}`, 'error');
